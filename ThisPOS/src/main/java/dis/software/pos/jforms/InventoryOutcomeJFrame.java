...
import dis.software.pos.mail.processors.InventoryOutcomeMailProcessor;
...
public class InventoryOutcomeJFrame extends javax.swing.JInternalFrame
{
    ...
    private static final Logger logger = LogManager.getLogger(InventoryOutcomeJFrame.class.getSimpleName());
    ...
    public InventoryOutcomeJFrame()
    {
        ...
        InventoryOutcomeJFrame frame = this;
    ...
    private Boolean isJframeValid()
    {
        ...
        {
            String jlblMsg = "Ingrese los datos marcados con un asterisco para agregar el registro.";
            OptionPane.showMessageDialog(this, jlblMsg, " Agregar registro", JOptionPane.INFORMATION_MESSAGE);
    ...
    private void jbtnSaveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jbtnSaveMouseClicked
        ...
        String jlblMsg = "¿Está seguro de que quiere efectuar la salida de estos productos?";
        if (OptionPane.showConfirmDialog(this, jlblMsg, " Guardar registro", JOptionPane.YES_NO_OPTION, 
        ...
        if (inventory.getId() != null)
        {
            ...
            if (ApplicationSession.getNotifications().stream()
                .anyMatch(x -> x.getCode().equals(Notification.INVENTORY_OUTCOME)
                    && x.getStatus().equals(EntityStatus.ACTIVE)))
            {
                ApplicationSession.getThreadPoolExecutor().execute(new InventoryOutcomeMailProcessor(inventory));
            }
