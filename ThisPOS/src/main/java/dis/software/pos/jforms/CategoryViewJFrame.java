...
/**
 * Formulario para la visualización de categorías
...
public class CategoryViewJFrame extends javax.swing.JInternalFrame
{
    ...
    public CategoryViewJFrame()
    {
        ...
        CategoryTableModel categoryTableModel = new CategoryTableModel(iCategory.findAll());
        
        jtableCategories.setModel(categoryTableModel);
        jtableCategories.getTableHeader().setFont(new Font("Segoe UI", Font.BOLD, 14));
        //Se habilitan las lineas horizontales para distinguir los registros
        jtableCategories.setShowHorizontalLines(Boolean.TRUE);
        jtableCategories.setGridColor(new Color(179, 179, 179));
        
        sorter = new TableRowSorter<>(categoryTableModel);
        jtableCategories.setRowSorter(sorter);
        
        //Se eliminan columnas innecesarias para esta vista
        jtableCategories.getColumnModel().removeColumn(jtableCategories.getColumnModel().getColumn(
            jtableCategories.convertColumnIndexToView(CategoryTableModel.COLUMN_ID)));
        ...
        list.add(new TableFilterBy(CategoryTableModel.COLUMN_CODE, "Código"));
        list.add(new TableFilterBy(CategoryTableModel.COLUMN_NAME, "Nombre"));
        list.add(new TableFilterBy(CategoryTableModel.COLUMN_DESC, "Descripción"));
        jcboFilterBy.setModel(new TableFilterByComboBoxModel(list));
        jcboFilterBy.setRenderer(new TableFilterByComboBoxRenderer());
        
        jtxtTotalRows.setText(Integer.toString(categoryTableModel.getRowCount()));
    ...
    private void filter()
    {
        ...
        RowFilter<CategoryTableModel, Object> descRowFilter = null;
        try
        {
    ...
    private void edit()
    {
        ...
        Category category = ((CategoryTableModel) jtableCategories.getModel()).get(
            jtableCategories.convertRowIndexToModel(jtableCategories.getSelectedRow()));

        CategoryEditJFrame categoryEditJFrame = new CategoryEditJFrame(category);
        getDesktopPane().add(categoryEditJFrame);
        categoryEditJFrame.pack();
        try
        {
            categoryEditJFrame.setMaximum(true);
        }
        catch (PropertyVetoException ex)
        {
            //logger.error("Error setting maximum", ex);
        }
        categoryEditJFrame.setVisible(true);
    ...
    private void delete()
    {
        ...
        CategoryTableModel categoryTableModel = (CategoryTableModel) jtableCategories.getModel();
        ...
        Category category = iCategory.findById(categoryTableModel.get(
            jtableCategories.convertRowIndexToModel(jtableCategories.getSelectedRow())).getId());
        category.setDeleted(Boolean.TRUE);
        category.setUpdatedBy(ApplicationSession.getUser());
        category.setUpdatedDate(new GregorianCalendar());
